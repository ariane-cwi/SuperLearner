...
#
SuperLearnerPseudo <- function(t, d, X, newX = NULL, tmax = NULL, family = gaussian(), SL.library,
  ...
  if (sum(is.na(X)) > 0 | sum(is.na(newX)) > 0 | sum(is.na(t)) > 0 | sum(is.na(d)) > 0) {
    stop("missing data is currently not supported. Check t, d, X, and newX for missing values")
  }
  if (!is.numeric(t)) {
    stop("the outcome t must be a numeric vector")
  }
  if (!is.numeric(d)) {
    stop("the outcome d must be a numeric vector")
  }
  if(is.null(tmax)){
    tmax <- max(t[d==1])
  }
  ...
  # create CV folds
  validRows <- CVFolds(N = N, id = id, Y = t, cvControl = cvControl)
  ...
  if(!identical(length(id), N)) {
    stop("id vector must have the same dimension as t")
  }
  ...
  if(!identical(length(obsWeights), N)) {
    stop("obsWeights vector must have the same dimension as t")
  }
  
  # create function for the cross-validation step:
  .crossValFUN <- function(valid, t, d, dataX, tmax, id, obsWeights, library, 
                           kScreen, k, p, libraryNames, saveCVFitLibrary) {
    # A random sample of the same size as valid is withdrawn from the 
    # learning set to compute pseudo-observations for points in valid
    KM <- sample(setdiff(1:length(t),valid),length(valid))
    # pseudo-observations
    pseudo <- pseudomean2(t[valid],d[valid],tmax,timeKM=t[KM],eventKM=d[KM])$pseudo
    
    tempLearn <- dataX[-c(valid,KM), , drop = FALSE]
    tempOutcome <- pseudomean(t[-c(valid,KM)],d[-c(valid,KM)],tmax)
    tempValid <- dataX[valid, , drop = FALSE]
    tempWhichScreen <- matrix(NA, nrow = kScreen, ncol = p)
    tempId <- id[-c(valid,KM)]
    tempObsWeights <- obsWeights[-c(valid,KM)]
    ...
    invisible(list(out = out, pseudo = pseudo, model_out = model_out))
  }
  ...
  crossValFUN_out <- lapply(validRows, FUN = .crossValFUN, 
                            t = t, d = d, dataX = X, tmax = tmax, id = id, 
  ...
  pseudo[unlist(validRows, use.names = FALSE)] <- unlist(lapply(crossValFUN_out, "[[", "pseudo"))
  ...
  getCoef <- method$computeCoef(Z = Z, Y = pseudo, libraryNames = libraryNames,
